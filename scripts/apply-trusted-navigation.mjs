...
for (const [zoneSlug, entries] of entriesByZone) {
  ...
  const section = `<section class="prose" data-trusted-zone-list="true"><h2>${ready.length} discovery-ready ${ready.length === 1 ? "protocol" : "protocols"}</h2>${empty}${reviewNote}</section>`;
...
for (const file of pagesToUpdate) {
  ...
  for (const [zoneSlug, stats] of Object.entries(summary.zones)) {
    ...
    const pattern = new RegExp(`(<article class="card"><span class="card-label">)[^<]*(</span><h3><a href="${zoneLink.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}">)`);
  ...
  for (const [areaSlug, stats] of Object.entries(summary.areas)) {
    ...
    const pattern = new RegExp(`(<article class="card"><span class="card-label">)[^<]*(</span><h3><a href="${areaLink.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}">)`);
