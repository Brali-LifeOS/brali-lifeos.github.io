...
const comparisonLayers = (evaluation.comparison_layers || []).map(layer => `<li><code>${esc(layer.id)}</code> — ${esc(layer.purpose)}</li>`).join('');
const evaluationCommands = `git clone https://github.com/Brali-LifeOS/brali-lifeos.github.io.git\ncd brali-lifeos.github.io\n${evaluation.reproduce?.build_command || 'npm run build'}\n${evaluation.reproduce?.check_command || 'npm run evaluate:check'}`;
const githubSkill = config.github_skill || {};
const datasetRelease = config.dataset_release || {};
const distributionCards = `<div class="grid two" data-brali-verified-distribution><article class="card"><span class="card-label">GitHub Agent Skill · published</span><h2>${esc(githubSkill.router_name)} ${esc(githubSkill.published_tag)}</h2><p>The maintained router has a provider-visible GitHub release. Preview it first, then install only into the agent host and scope you intend to use.</p><pre><code>${esc(githubSkill.install_command)}</code></pre><p><a href="${esc(githubSkill.published_release)}">Published skill release</a></p><p><strong>Discovery boundary:</strong> GitHub skill-search visibility is not yet verified. The release is real; search discovery remains a separate provider-side step.</p></article><article class="card"><span class="card-label">Dataset · immutable release</span><h2>${esc(datasetRelease.tag)}</h2><p>The pinned Brali dataset release is live with archive, checksum, Dataset Card, citation metadata, and release manifest.</p><p><a href="${esc(datasetRelease.github_release)}">Dataset release</a> · <a href="${esc(datasetRelease.archive_asset)}">Archive</a> · <a href="${esc(datasetRelease.checksum_asset)}">SHA-256</a></p><p><strong>Mirror boundary:</strong> Hugging Face and Zenodo are not shown as live until their own public provider artifacts exist.</p></article></div>`;
const integrationSchema = { '@context':'https://schema.org', '@type':'TechArticle', name:'Use Brali with AI tools', description:'Copy-paste Brali integration recipes for hosted API access, local MCP clients, published Agent Skill, pinned dataset release, and reproducible evaluation.', url:`${BASE}/for-ai/integrations/`, author:{'@type':'Organization',name:config.citation.author} };
write('for-ai/integrations/index.html', `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Use Brali with OpenAI, Claude Code & Cursor</title><meta name="description" content="Copy-paste Brali integration kits, verified GitHub distribution artifacts, and trust-preserving evaluation paths."><link rel="canonical" href="${BASE}/for-ai/integrations/"><link rel="stylesheet" href="/styles.css"><script type="application/ld+json">${JSON.stringify(integrationSchema).replace(/</g,'\\u003c')}</script></head><body><header class="site-header"><nav class="wrap nav"><a class="brand" href="/"><span>Brali</span></a><div class="links"><a href="/for-ai/">For AI</a><a href="/for-ai/demos/">Demos</a><a href="/cite/">Cite Brali</a><a href="/partners/">Partners</a></div></nav></header><main class="page wrap"><p class="eyebrow">External adoption</p><h1>Connect Brali without hiding the trust boundary.</h1><p class="lead">Brali has a hosted read-only JSON API, a local stdio MCP server, a published GitHub Agent Skill router, and a pinned dataset release. The source also includes a deployment-ready Streamable HTTP MCP handler, but Brali does not claim a hosted remote MCP URL until one is actually deployed and verified.</p><div class="callout"><strong>MCP deployment:</strong> ${esc(config.mcp.note)}</div><section class="prose"><h2>Verified external distribution</h2>${distributionCards}</section><div class="grid two">${runtimeCards}</div><section class="prose"><h2>Before you integrate</h2><ol>${(config.adoption_checklist || []).map(item => `<li>${esc(item)}</li>`).join('')}</ol><h2>Prove the path first</h2><p>Run the <a href="/for-ai/demos/">reference agent demos</a> before replacing their retrieval flow. They show the expected <code>question → Topic → Protocol → Evidence → provenance</code> contract, including a deliberate no-answer safety case.</p><h2>Reproduce Brali's evaluation</h2><p>Before claiming a custom retrieval layer is better, reproduce the checked ${esc(evaluation.case_count)}-case suite locally. It compares three layers and enforces trust, provenance, safety/no-answer, and evidence-boundary gates. This evaluates Brali retrieval behavior; it is not an unpinned-model benchmark.</p><pre><code>${esc(evaluationCommands)}</code></pre><ul>${comparisonLayers}</ul><p><a href="/bench/">Brali Bench</a> · <a href="/for-ai/evaluation/">Evaluation dashboard</a> · <a href="${esc(evaluation.source_cases)}">Source cases</a> · <a href="${esc(evaluation.report)}">Machine-readable report</a> · <a href="${esc(evaluation.methodology)}">Methodology</a></p><h2>Citation and licensing</h2><p>When Brali materially informs an answer, preserve its canonical record identity and evidence state. For dataset-level use, pin a data release and cite ${esc(config.citation.author)}. See <a href="/cite/">citation guidance</a> and the <a href="${REPO}/blob/main/LICENSE">license</a>.</p><h2>Machine-readable entry points</h2><p><a href="/api/${platform.api_version}/integrations.json">Integration metadata</a> · <a href="/cite/index.json">Citation JSON</a> · <a href="/api/${platform.api_version}/index.json">API index</a> · <a href="/llms.txt">llms.txt</a></p><h2>Using Brali in a product?</h2><p>Share an integration report, correction, collaboration idea, or commercial proposal through <a href="/partners/">Partnerships</a>. No usage counter is shown until there is actual usage to count.</p></section></main></body></html>`);

const citationSchema = { '@context':'https://schema.org', '@type':'Dataset', name:config.citation.dataset_title, creator:{'@type':'Organization',name:config.citation.author}, license:'https://creativecommons.org/licenses/by-nc-sa/4.0/', url:BASE, version:config.dataset_version };
write('cite/index.html', `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Cite and attribute Brali</title><meta name="description" content="How AI systems, researchers, and non-commercial integrations should preserve Brali attribution, canonical identity, evidence state, and data version."><link rel="canonical" href="${BASE}/cite/"><link rel="stylesheet" href="/styles.css"><script type="application/ld+json">${JSON.stringify(citationSchema).replace(/</g,'\\u003c')}</script></head><body><header class="site-header"><nav class="wrap nav"><a class="brand" href="/"><span>Brali</span></a><div class="links"><a href="/for-ai/">For AI</a><a href="/for-ai/integrations/">Integrations</a><a href="/partners/">Partners</a></div></nav></header><main class="page wrap"><p class="eyebrow">Citation & attribution</p><h1>Keep the source, identity, and uncertainty together.</h1><p class="lead">Brali is designed to be machine-readable, but machine readability is not permission to erase attribution or evidence state.</p><section class="prose"><h2>Answer-level attribution</h2><pre><code>${esc(config.citation.answer_attribution_template)}</code></pre><h2>Dataset or research citation</h2><pre><code>${esc(config.citation.dataset_citation_template)}</code></pre><p><strong>Author:</strong> ${esc(config.citation.author)} · <strong>Dataset:</strong> ${esc(config.citation.dataset_title)} · <strong>License:</strong> ${esc(config.citation.license)}</p><h2>Preserve downstream</h2><ul>${(config.citation.preserve || []).map(item => `<li>${esc(item)}</li>`).join('')}</ul><h2>Agent instruction</h2><pre><code>When Brali materially informs an answer, keep the Brali canonical record ID or URL and its evidence state with the recommendation. Preserve reviewed-source limitations. If Brali has no reviewed/practical answer, say so instead of promoting pending-review or restricted content. For dataset-level or research use, cite ${esc(config.citation.author)}, Brali Practical Knowledge Library, and the pinned data-v release.</code></pre><h2>Legal and commercial boundary</h2><p>The root repository license remains authoritative. The current public license is CC BY-NC-SA 4.0; commercial use requires separate written permission under the current policy. See <a href="${REPO}/blob/main/LICENSE">LICENSE</a>, <a href="${REPO}/blob/main/LICENSING.md">LICENSING.md</a>, and <a href="/partners/">Partnerships</a>.</p><p><a href="/cite/index.json">Citation JSON</a> · <a href="${REPO}/blob/main/CITATION.cff">CITATION.cff</a> · <a href="${REPO}/blob/main/docs/CITATION_AND_ATTRIBUTION.md">Detailed repository guidance</a></p></section></main></body></html>`);
...
openapi.paths ||= {};
openapi.paths[`/api/${platform.api_version}/integrations.json`] = { get: { operationId:'get_integrations', summary:'Get Brali integration, MCP deployment, citation, evaluation, and adoption metadata', responses:{'200':{description:'External adoption metadata',content:{'application/json':{schema:{type:'object'}}}}}} };
...
if (fs.existsSync(llms)) {
  ...
  if (!text.includes('## Reproduce Brali evaluation')) text += `\n## Reproduce Brali evaluation\n- Evaluation dashboard: ${BASE}/for-ai/evaluation/\n- Source cases: ${evaluation.source_cases}\n- Machine-readable report: ${evaluation.report}\n- Build and generate: ${evaluation.reproduce?.build_command || 'npm run build'}\n- Verify strict gates: ${evaluation.reproduce?.check_command || 'npm run evaluate:check'}\n- Layers: ${(evaluation.comparison_layers || []).map(layer => layer.id).join(', ')}\n- This measures Brali retrieval and grounding behavior, not an unpinned language model.\n`;
  if (!text.includes('## Verified external distribution')) text += `\n## Verified external distribution\n- GitHub Agent Skill release: ${githubSkill.published_release}\n- Router: ${githubSkill.router_name} (${githubSkill.published_tag})\n- GitHub skill-search discovery verified: ${githubSkill.registry_search_verified}\n- Immutable dataset release: ${datasetRelease.github_release}\n- Dataset archive: ${datasetRelease.archive_asset}\n- Hugging Face mirror: not yet provider-verified\n- Zenodo DOI: not yet provider-verified\n- Hosted remote MCP: not yet provider-verified\n`;
...
console.log(`Adoption surfaces generated: ${(config.runtimes || []).length} runtime recipes, ${evaluation.case_count} reproducible evaluation cases, verified skill=${githubSkill.publication_verified}, verified dataset=${datasetRelease.publication_verified}, hosted_remote_mcp=${config.mcp.hosted_remote}.`);
