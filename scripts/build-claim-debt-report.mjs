...
const ontology = await readJson('data/knowledge-ontology.json');
...
const topicDebtCards = topicDebt.slice(0, 12).map(([topicId, count], index) => {
  const title = topicTitleById.get(topicId) ?? topicId;
  return `<article class="card" data-claim-topic-rank="${index + 1}" data-claim-topic="${escapeHtml(topicId)}" data-claim-topic-count="${count}"><span class="card-label">#${index + 1} · ${count} debt record${count === 1 ? '' : 's'}</span><h3>${escapeHtml(title)}</h3><p><code>${escapeHtml(topicId)}</code></p><p>Editorial workload only. This count is not a popularity, risk, or evidence-strength score.</p></article>`;
}).join('');
...
const queueCards = priorityDebt.map((queueEntry, index) => {
  ...
  const topicTitles = (debt.topic_ids ?? []).map(topicId => topicTitleById.get(topicId) ?? topicId);
  ...
  return `<article class="card" data-claim-queue-rank="${index + 1}" data-claim-queue-slug="${escapeHtml(queueEntry.slug)}"><span class="card-label">#${index + 1} · ${escapeHtml(humanize(debt.status))} · ${escapeHtml(zoneTitle)}</span><h3>${escapeHtml(title)}</h3><p><strong>Topics:</strong> ${escapeHtml(topicTitles.join(' · ') || 'Topic pending')}</p><p><strong>Debt:</strong> ${escapeHtml(reasons || 'Editorial review required')}</p><p><strong>Markers:</strong> ${escapeHtml(categories || 'None')}</p><p><strong>Priority signals:</strong> ${escapeHtml(factors || 'Queue order')}</p></article>`;
}).join('');
...
const schema = {
  ...
  description: 'A transparent summary of claim-review markers, Topic-level unresolved claim debt, indexing safeguards, and the current editorial queue in Brali.',
  url: canonical,
...
if (!methodology.includes('/quality/claims/')) {
  methodology = methodology.replace(
    /<p><a href="\/life-os\/datasets\/claim-debt\.json">[^<]*<\/a><\/p>/,
...
console.log(`Public claim debt report built: ${counts.records_checked ?? 0} checked, ${counts.debt_entries ?? 0} unresolved, ${counts.indexable_debt_entries ?? 0} indexable, ${topicDebt.length} Topic debt group(s), ${priorityDebt.length} priority rows.`);
