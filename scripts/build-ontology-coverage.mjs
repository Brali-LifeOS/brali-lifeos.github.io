...
const cards = `<div class="grid three"><article class="card"><span class="card-label">Library</span><h3>${records.length} entries</h3><p>${topicMapped} have a concrete Topic; ${topicPendingRecords.length} still require Topic classification.</p></article><article class="card"><span class="card-label">Coverage</span><h3>${report.summary.topic_coverage_percent}% Topic-mapped</h3><p>Every entry has a Domain and at least one semantic classification from the compatibility layer.</p></article><article class="card"><span class="card-label">Research backlog</span><h3>${growthGaps.length} growth-gap Topics</h3><p>${report.summary.research_candidates} current research candidates can help close or challenge gaps.</p></article></div>`;
...
console.log(`Ontology coverage generated: ${topicMapped}/${records.length} Topic-mapped; ${topicPendingRecords.length} pending; ${growthGaps.length} growth-gap Topics.`);
