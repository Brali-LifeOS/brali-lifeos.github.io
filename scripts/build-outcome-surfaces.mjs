...
if (policy.collection_status !== 'instrumented') fail('outcome status must be instrumented once explicit Query feedback is live');
if (policy.instrumentation_scope?.query_feedback !== 'live') fail('Query feedback instrumentation status drift');
if (policy.instrumentation_scope?.protocol_execution !== 'not-live') fail('Protocol execution must remain explicitly not-live until a runner exists');
...
const report = {
  ...
  interpretation: registry.observations.length
    ...
    : 'Ask Brali can now create explicit opt-in Query feedback events, but no reviewed observation has been accepted yet. Zero remains an observed zero; drafts, downloads, share actions, page views and CI do not count as use.',
  privacy: {
  ...
  next_instrumentation_step: 'Add a browser Protocol Runner that can create protocol_started, protocol_completed and linked helpful feedback envelopes without automatic transmission.'
};
...
if (!datasets.includes('/quality/outcomes/')) {
  const block = '<aside class="callout" data-brali-outcomes><h3>Observed outcomes</h3><p>See the privacy-light feedback contract, reviewed registry, and current observed counts.</p><a class="button" href="/quality/outcomes/">Open outcome report</a></aside>';
...
console.log(`Outcome surfaces built: status=${policy.collection_status}; query-feedback=live; protocol-execution=not-live; reviewed-events=${registry.observations.length}; verified-successes=${verifiedRuns.length}; automatic-collection=false.`);
