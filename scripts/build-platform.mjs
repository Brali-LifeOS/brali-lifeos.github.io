...
writeJson(`${apiDir}/topics.json`, { schema_version: 1, dataset_version: config.dataset_version, attribution, items: apiTopics }); writeJson(`${apiDir}/protocols.json`, { schema_version: 1, dataset_version: config.dataset_version, attribution, items: apiProtocols }); writeJson(`${apiDir}/hacks.json`, { schema_version: 1, dataset_version: config.dataset_version, attribution, items: apiHacks }); writeJson(`${apiDir}/evidence.json`, { schema_version: 1, dataset_version: config.dataset_version, attribution, items: apiEvidence }); writeJson(`${apiDir}/search.json`, { schema_version: 1, dataset_version: config.dataset_version, attribution, note: 'Compact client-side retrieval index. Consumers should preserve trust/evidence state, canonical URL, and attribution.', items: searchDocs.map(({ text, ...doc }) => ({ ...doc, search_text: text })) }); writeJson(`${apiDir}/identity.json`, { schema_version: 1, dataset_version: config.dataset_version, attribution, identities, aliases });
writeJson(`${apiDir}/index.json`, { name: 'Brali Knowledge API', api_version: config.api_version, dataset_version: config.dataset_version, static_api: true, attribution, endpoints: ['topics.json', 'protocols.json', 'hacks.json', 'evidence.json', 'search.json', 'identity.json', 'manifest.json', 'openapi.json'], semantics: { lookup: 'Filter list files by canonical_id or local id.', trust: 'Only reviewed/practical protocols are normal trusted recommendations. Preserve evidence state in downstream output.', citation: 'Preserve the canonical Brali record URL and Brali — Dzmitryi Kharlanau attribution when the interface permits citations or source labels.', missing: 'An empty filtered result means not found; clients must not invent a match.' } });
const openapi = { openapi: '3.1.1', info: { title: 'Brali Knowledge API', version: config.dataset_version, description: 'Versioned read-only static JSON API over the canonical Brali knowledge model.' }, servers: [{ url: 'https://brali-lifeos.github.io' }], paths: {}, 'x-brali-attribution': attribution };
