...
for (const scenario of config.scenarios || []) {
  ...
  results.push({
    id: scenario.id,
    expectations: {
      expected_topic_ids: scenario.expected_topic_ids || [],
      expected_protocol_slugs: scenario.expected_protocol_slugs || [],
      expected_decision_ids: scenario.expected_decision_ids || [],
      expected_status: scenario.expected_status,
      safety_boundary: Boolean(scenario.safety_boundary),
      coverage_gap: Boolean(scenario.coverage_gap),
    },
    packet,
    mcp_plan: buildMcpPlan(packet),
  });
}
const dataset = {
  schema_version: 1,
  ...
  description: 'Deterministic practical-agent examples generated from the same static API and trust rules external consumers can use, including explicit trusted-coverage and safety no-answer cases.',
...
const cards = results.map(({ id, packet, expectations }) => {
  ...
  const explicitBoundary = expectations.coverage_gap
    ? '<p><strong>Coverage gap:</strong> The Topic route is preserved, but the current trusted feed contains no eligible protocol. The demo returns no answer instead of falling back to withheld content.</p>'
    : expectations.safety_boundary
      ? '<p><strong>Safety boundary:</strong> This treatment request is blocked from normal practical retrieval.</p>'
      : '';
  return `<article class="card"><span class="card-label">${esc(packet.status)}</span><h2>${esc(id)}</h2><p><strong>Question:</strong> ${esc(packet.question)}</p><p><strong>Route:</strong> ${topics}</p>${explicitBoundary}<h3>Trusted recommendations</h3><ul>${recs}</ul><h3>Evidence boundaries</h3><ul>${boundaries}</ul></article>`;
}).join('');
...
write('for-ai/demos/index.html', `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Reference agent demos | Brali</title><meta name="description" content="Runnable Brali API and MCP reference scenarios showing Topic routing, trusted Protocols, Evidence Decisions, provenance, trusted-coverage gaps, and safe no-answer behavior."><link rel="canonical" href="${dataset.page_url}"><link rel="stylesheet" href="/styles.css"><script type="application/ld+json">${JSON.stringify(schema).replace(/</g,'\\u003c')}</script></head><body><header class="site-header"><nav class="wrap nav"><a class="brand" href="/"><span>Brali</span></a><div class="links"><a href="/for-ai/">For AI</a><a href="/topics/">Topics</a><a href="/life-os/datasets/">Datasets</a></div></nav></header><main class="page wrap"><p class="eyebrow">Reference integration</p><h1>Question → Topic → Protocol → Evidence → provenance</h1><p class="lead">These examples are generated during the Brali build from the same API files an external agent reads. They are regression fixtures, not hand-authored success screenshots. A routed Topic can still return no trusted answer when coverage has not earned normal discovery.</p><div class="callout"><strong>Contract:</strong> API ${esc(dataset.api_version)} · dataset ${esc(dataset.dataset_version)}. Normal recommendations are limited to <code>reviewed</code>/<code>practical</code> content. The Task Initiation scenario exposes a current trusted-coverage gap; the safety scenario blocks a treatment request.</div><div class="grid two">${cards}</div><section class="prose"><h2>Run it</h2><pre><code>npm run build
...
for (const rel of ['for-ai/index.html', 'life-os/datasets/index.html']) {
  ...
  if (!html.includes('data-reference-agent-demos')) html = html.replace('</main>', '<aside class="callout" data-reference-agent-demos><h3>Runnable reference agent demos</h3><p>See generated Sleep, Memory, Task Initiation, and safety-boundary examples, including explicit trusted-coverage and safety no-answer cases.</p><a class="button" href="/for-ai/demos/">Open demos</a></aside></main>');
...
console.log(`Reference agent demos generated: ${results.length} scenarios; ${results.filter(x=>x.packet.status==='trusted-answer').length} trusted answers; ${results.filter(x=>x.packet.status==='no-trusted-answer').length} explicit no-answer(s).`);
