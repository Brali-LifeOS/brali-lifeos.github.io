...
const configCheckOnly = args.includes('--config-check');
...
if (configCheckOnly) {
  console.log(`Search Console measurement contract verified: ${config.segments.length} stable segments, ${trusted.size} trusted protocol slugs, ${problems.collections.length} canonical problems.`);
  process.exit(0);
}
...
const measured = {
  ...
  source_report: path.relative(ROOT, reportPath).replaceAll('\\', '/'),
...
fs.writeFileSync(path.join(outputDir, 'brali-search-measurement.md'), `${lines.join('\n')}\n`);

console.log(`Brali Search measurement: status=${measured.measurement_status}; tracked=${measured.segments.reduce((sum, segment) => sum + segment.tracked_pages, 0)}; top-query opportunities=${opportunities.length}.`);
