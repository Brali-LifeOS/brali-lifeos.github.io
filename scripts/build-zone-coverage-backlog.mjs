...
const output = {
  schema_version: 1,
  purpose: 'Editorial backlog for Growth Zones that currently have zero trusted protocols. Candidate scores are triage-only and never change evidence status automatically.',
...
console.log(`Zone coverage backlog: ${rows.length} zero-trust zones.`);
for (const row of rows) {
  const best = row.candidates[0];
  console.log(`ZONE_COVERAGE ${row.zone_slug} | ${row.zone_title} | entries=${row.entry_count} | best=${best?.slug ?? 'none'} | status=${best?.status ?? 'none'} | sensitive=${best?.sensitive ?? false} | quantitative=${best?.quantitative_claims ?? false} | evidenceLanguage=${best?.evidence_language ?? false} | source=${best?.source_recorded ?? false} | score=${best?.score ?? 0}`);
}
