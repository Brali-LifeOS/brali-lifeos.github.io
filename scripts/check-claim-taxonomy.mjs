...
if (fixtures.schema_version !== 1) fail(`unexpected fixture schema_version ${fixtures.schema_version}`);
...
for (const definition of claimCategoryDefinitions) {
  if (!definition.id || typeof definition.enforced !== 'boolean' || !definition.description) fail('malformed category definition');
}

for (const testCase of fixtures.cases) {
  if (!testCase.id || ids.has(testCase.id)) fail(`missing or duplicate fixture id ${testCase.id}`);
  ids.add(testCase.id);
  for (const category of [...(testCase.expected_categories ?? []), ...(testCase.expected_enforced_categories ?? [])]) {
...
console.log(`Claim taxonomy verified: ${claimCategoryDefinitions.length} categories across ${fixtures.cases.length} regression cases.`);
