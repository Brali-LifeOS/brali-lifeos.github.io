...
export function validateProductIdentity() {
  ...
  assert(adoption.product_identity?.publisher === 'MetalHeadsCats', 'Product identity must be maintained by MetalHeadsCats.');
  ...
  assert(readme.includes('MetalHeadsCats, Brali Practical Knowledge Library'), 'README citation must match the maintained public entity identity.');
  assert(!readme.includes('MetalHatsCats'), 'README must not drift back to the legacy Brali maintainer spelling.');
