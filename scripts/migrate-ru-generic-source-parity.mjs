...
const reviewed = new Map([
  ...
  ["if-then-rules-productivity", {
    ...
    description: "Для цели, которую вы уже хотите реализовать, выберите конкретный наблюдаемый сигнал и выполнимое действие: «если происходит X, то я делаю Y». Такие implementation intentions хорошо изучены как инструмент саморегуляции, но не гарантируют автоматического поведения и требуют реалистичного действия и подходящего контекста.",
  }],
